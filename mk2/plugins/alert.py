...
class Alert(Plugin):
    ...
    def setup(self):
        ...
        self.register(self.server_started, Hook, public=True,
                      name="reload-alert",
                      doc='Reload all of the alerts from %s' % self.path)
    ...
    def server_started(self, event=None):
        if self.path and os.path.exists(self.path):
            ...
            if self.messages and not self.repeating:
                ...
                self.repeating = True
            if self.messages:
                self.console("[Plugin] Alert: Successfully loaded %s" %
                             self.path)
            else:
    ...
    def repeater(self, event):
        if not self.empty and self.messages:
            self.send_format(self.command, parseColors=True,
